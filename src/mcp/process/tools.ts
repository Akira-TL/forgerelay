  ...
  registerAppTool(
    ...
    toolNames.shell,
    {
      ...
      inputSchema: {
        ...
        yieldTimeMs: z.number().int().min(0).max(300_000).optional().describe("Feedback wait. action=run defaults to 10000ms; 0 hands off immediately. Wait-only action=process defaults to 60000ms; positive shorter values are raised to 60000ms and buffered output does not shorten the wait. Use 0 once per running process for one immediate wait-only status probe; repeated wait-only 0 is rejected. Interactive action=process defaults to 250ms. Process exit returns sooner."),
