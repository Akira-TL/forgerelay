  ...
  registerAppTool(
    ...
    "write_stdin",
    {
      ...
      inputSchema: {
        ...
        yieldTimeMs: z.number().int().min(0).max(300_000).optional().describe("Milliseconds to keep waiting before returning again, max 300000. Polling defaults to 5000; interaction defaults to 250."),
        maxOutputTokens: z.number().int().positive().max(100_000).optional().describe("Approximate output token budget. Defaults to 10000."),
      },
      outputSchema: processOutputSchema(),
      _meta: {},
