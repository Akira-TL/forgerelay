  ...
  registerAppTool(
    ...
    toolNames.shell,
    {
      ...
      inputSchema: {
        ...
        yieldTimeMs: z.number().int().min(0).max(300_000).optional().describe("Feedback wait. action=run defaults to 10000ms; 0 hands off immediately. Wait-only action=process defaults to 60000ms; positive shorter values are raised to 60000ms and buffered output does not shorten the wait. Use 0 only for one immediate status probe. Interactive action=process defaults to 250ms. Process exit returns sooner."),
  ...
  registerAppTool(
    ...
    "write_stdin",
    {
      ...
      inputSchema: {
        ...
        yieldTimeMs: z.number().int().min(0).max(300_000).optional().describe("Wait window, max 300000ms. Wait-only polling defaults to 60000ms; positive shorter values are raised to 60000ms and buffered output does not shorten the wait. Use 0 only for one immediate status probe. Interaction defaults to 250ms; process exit returns sooner."),
        maxOutputTokens: z.number().int().positive().max(100_000).optional().describe("Approximate output token budget. Defaults to 10000."),
      },
      outputSchema: processOutputSchema(),
      _meta: {},
