...
function capabilityContractInstructions(config: ServerConfig): string {
  ...
  const workspaceLifecycle = `Use ForgeRelay as a local coding workspace. Default to the user's existing checkout. Reuse workspaceId from ${toolNames.openWorkspace}; change it only when asked.${staleWorkspacePolicy} Only open mode=\"worktree\" when the user explicitly asks for isolated or parallel Git work. ${toolNames.closeWorkspace} preserves Workspace identity; managed close finalizes/removes backing and requires commitMessage. action=delete removes ForgeRelay state; active worktrees still finalize safely; checkout files are never deleted.`;
