...
test("cross-platform cloud CI splits shared tests from bounded platform acceptance", async () => {
  ...
  assert.match(windowsPlatform, /if:\s*steps\.windows-platform-deps\.outputs\.cache-hit != 'true'/);
  assert.match(windowsPlatform, /run:\s*npx --yes npm@11\.19\.1 ci --no-audit --no-fund/);
  assert.match(windowsPlatform, /run:\s*npx --yes npm@11\.19\.1 run ci:platform-tests -- \$\{\{ matrix\.shard \}\}/);
  assert.doesNotMatch(windowsPlatform, /npm install --global npm@11\.19\.1/);
