...
test("cross-platform cloud CI splits shared tests from bounded platform acceptance", async () => {
  ...
  assert.ok(windowsPlatform, "Windows platform job must remain present");
  assert.match(windowsPlatform, /uses:\s*actions\/cache@v4/);
  assert.match(windowsPlatform, /path:\s*node_modules/);
  assert.match(windowsPlatform, /runner\.os/);
  assert.match(windowsPlatform, /runner\.arch/);
  assert.match(windowsPlatform, /hashFiles\('\.nvmrc'\)/);
  assert.match(windowsPlatform, /npm-11\.19\.1/);
  assert.match(windowsPlatform, /hashFiles\('package-lock\.json'\)/);
  assert.match(windowsPlatform, /id:\s*windows-npm-cli[\s\S]*path:\s*\.ci-npm-global/);
  assert.match(windowsPlatform, /if:\s*steps\.windows-npm-cli\.outputs\.cache-hit != 'true'[\s\S]*npm install --global --prefix \.ci-npm-global npm@11\.19\.1/);
  assert.match(windowsPlatform, /if:\s*steps\.windows-platform-deps\.outputs\.cache-hit != 'true'[\s\S]*node \.ci-npm-global\/node_modules\/npm\/bin\/npm-cli\.js ci --no-audit --no-fund/);
  assert.match(windowsPlatform, /run:\s*node \.ci-npm-global\/node_modules\/npm\/bin\/npm-cli\.js run ci:platform-tests -- \$\{\{ matrix\.shard \}\}/);
  assert.doesNotMatch(windowsPlatform, /npx --yes npm@11\.19\.1/);
