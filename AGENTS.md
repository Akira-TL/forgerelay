...
1. **The host is the orchestrator.** ForgeRelay exposes clear capabilities and execution state. It should not hide the workflow inside an opaque, uninspectable agent loop.
...
- **Server** — the local ForgeRelay MCP server.
...
Keep tunnel ownership and credentials with the user. ForgeRelay may operate through a user-controlled tunnel, but it does not own tunnel lifecycle or configuration.
...
Do not expand ForgeRelay's responsibility while fixing a local symptom. Host UI, provider model naming, tunnel management, and duplicated review experiences require an explicit product decision.
...
Every formal version tag, including release candidates, requires explicit developer manual acceptance before the tag is created or pushed. Automated tests, local release proof, packaged acceptance, and cloud CI are necessary verification but never substitute for a developer exercising the release candidate through the relevant real user-facing path and explicitly approving publication. If manual acceptance has not been reported as passed for the exact release-ready state, stop before tag creation or any other publication action.

Every formal version tag, including release candidates, must have dedicated release notes committed at `docs/releases/vX.Y.Z.md` (or the matching prerelease filename) before the release gate and tag are created. Do not rely only on `CHANGELOG.md`, autogenerated GitHub notes, or a commit list. Write the release notes as a user-facing summary of the version, including material behavior/configuration changes, migrations or security implications when relevant, and the verification that matters for shipping.

Do not create release notes speculatively during ordinary development. Create or update the dedicated file when a concrete version is being prepared, and keep it aligned with the version metadata that will actually be tagged.
...
Start at the boundary named by the problem and follow the data. Keep policy in ForgeRelay, provider translation in adapters, and important behavior in schemas, types, checks, or explicit tool results rather than hidden prompt conventions.
...
- Preserve host and provider data unless ForgeRelay has a concrete reason to normalize it.
