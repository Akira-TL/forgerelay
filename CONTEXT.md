...
用户通过命令行（CLI）直接连接远端锻造中继服务的命令行认证路由，并以现有所有者令牌（owner token）换取后续模型上下文协议（MCP）访问所需访问令牌和刷新令牌的过程。远端认证成功即建立或更新本机的远端实例记录，不再存在独立的“添加远端”步骤。安全外壳（SSH）不是认证前提，只在远端服务无法直接访问时提供可选传输路径。
避免混用：宿主开放授权（OAuth）、工作区接力（Workspace Relay）、安全外壳登录

**远端服务地址（Remote Service URL）**：
发起实例连接远端锻造中继服务时使用的模型上下文协议（MCP）服务地址。可以是本机可直接访问的域名或互联网协议地址（IP）；直接可达时优先使用该地址，不启动安全外壳（SSH）。
避免混用：锻造中继别名（Forge alias）、安全外壳目标（SSH target）

**安全外壳目标（SSH target）**：
仅在远端服务无法直接访问时交给系统安全外壳（SSH）客户端的可选连接目标。锻造中继只利用安全外壳端口转发（SSH Port Forwarding）把远端服务临时映射到本机回环地址；主机别名、密钥、代理和多级跳板（ProxyJump）全部沿用用户现有安全外壳配置，锻造中继不单独实现跳板能力。
避免混用：远端服务地址（Remote Service URL）、锻造中继别名（Forge alias）
